...
package GasExtra
  ...
  model Compressor_low "Gas compressor"
    ...
    parameter Real tablePhic_low[:, :]=fill(
          0,
          0,
          2) "Table for phic(N_T,beta)";
    parameter Real tableEta_low[:, :]=fill(
          0,
          0,
          2) "Table for eta(N_T,beta)";
    parameter Real tablePR_low[:, :]=fill(
          0,
          0,
          2) "Table for PR(N_T,beta)";
    ...
    Modelica.Blocks.Tables.CombiTable2D PressRatio_low(
      tableOnFile=if (Table == TableTypes.matrix) then false else true,
      table=tablePR_low,
      tableName=if (Table == TableTypes.matrix) then "NoName" else "tabPR_low",
      fileName=if (Table == TableTypes.matrix) then "NoName" else fileName,
      smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative)
    ...
    Modelica.Blocks.Tables.CombiTable2D Phic_low(
      tableOnFile=if (Table == TableTypes.matrix) then false else true,
      table=tablePhic_low,
      tableName=if (Table == TableTypes.matrix) then "NoName" else
          "tabPhic_low",
      fileName=if (Table == TableTypes.matrix) then "NoName" else fileName,
      smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative)
  ...
  equation
    ...
    eta_low = eta*(hout - gas_in.h)/(omega^2)
      "Efficiency definition at low speed";
    PR_low = (hout - gas_in.h)/(omega^2)
      "Pressure ratio definition at low speed";
...
<li><i>22 Apr 2005</i>
  ...
  model Compressor_M "Gas compressor"
    ...
    parameter Real tablePhic[:, :]=fill(
          ...
          0,
          2) "Table for phic(N_T,beta)";
    parameter Real tableM[:, :]=fill(
          0,
          0,
          2) "Table for torque(N_T,beta)";
    ...
    Modelica.Blocks.Tables.CombiTable2D Torque(
      ...
      fileName=if (Table == TableTypes.matrix) then "NoName" else fileName,
      table=tableM,
      tableName=if (Table == TableTypes.matrix) then "NoName" else "tabM",
      smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative)
      annotation (Placement(transformation(extent={{-10,60},{10,80}}, rotation=
              0)));
...
<li><i>22 Apr 2005</i>
  ...
  package Test
    ...
    equation
      ...
      connect(ConstantSpeed1.flange, Inertia1.flange_b) annotation (Line(points=
             {{52,-48},{68,-48},{68,-16},{54,-16}}, color={0,0,0}));
      connect(SourceW1.flange, GTunit1.Fuel_in) annotation (Line(points={{-18,
              52},{-14,52},{-14,21.04},{-12,21.04}}, color={159,159,223}));
      connect(SourceP1.flange, GTunit1.Air_in) annotation (Line(points={{-78,22},
...
double tabHI(12,4)   # table for HI_iso=h(ZLPout_iso,Tsync)
    ...
    4.85e6       16.224e6     15.2e6       15.6e6*/
      ...
      ThermoPower.Gas.SourceMassFlow
        ...
        T=291.44,
        p0=12.5e5,
        w0=0.365) annotation (Placement(transformation(extent={{-76,46},{-56,66}},
              rotation=0)));
      Electrical.Generator Generator(Np=2, eta=0.98) annotation (Placement(
            transformation(extent={{34,0},{54,20}}, rotation=0)));
      ...
      Modelica.Mechanics.Rotational.Components.IdealGear IdealGear1(ratio=(
            ...
            transformation(extent={{6,0},{26,20}}, rotation=0)));
    ...
    equation
      ...
      connect(SourceP1.flange, GTunit.Air_in) annotation (Line(points={{-78,22},
              {-78,21.3},{-69.8,21.3}}, color={159,159,223}));
      connect(GTunit.FlueGas_out, SinkP1.flange) annotation (Line(points={{-30.2,
              21.3},{-31.6,21.3},{-31.6,52},{-22,52}}, color={159,159,223}));
      ...
      connect(Breaker.connection2, Grid.connection) annotation (Line(
          ...
          pattern=LinePattern.None,
          thickness=0.5));
      connect(GTunit.shaft_b, Inertia.flange_a) annotation (Line(points={{-28.44,
              15},{-25.22,15},{-25.22,10},{-22,10}}, color={0,0,0}));
      ...
      connect(BooleanStep1.y, Breaker.closed)
        annotation (Line(points={{59,46},{68,46},{68,18}}, color={255,0,255}));
    initial equation
      Inertia.phi = 0;
      der(Inertia.w) = 0;
...
<p>Simulate for 2 s. The plant starts at steady states, and produces approximately 5 MW of power. At time t=1 the breaker opens, and the GT unit starts accelerating, with a time constant of 10 seconds.
    ...
    model Test_low
      ...
      Modelica.Mechanics.Rotational.Sources.ConstantSpeed ConstantSpeed1(
          w_fixed=5.23, useSupport=false) annotation (Placement(transformation(
              extent={{-92,-10},{-72,10}}, rotation=0)));
      ...
      Turbine_low Turbine_low1(
        ...
        pstart_out=2e5,
        pstart_in=7.3e5,
        Ndesign=523.3) annotation (Placement(transformation(extent={{6,12},{28,
                32}}, rotation=0)));
      Gas.PressDrop PressDrop1(
        ...
        wnom=0.3) annotation (Placement(transformation(extent={{34,20},{54,40}},
              rotation=0)));
      Gas.PressDrop PressDrop2(
        ...
        Tstart=244,
        wnom=0.3,
        dpnom=0.15e5) annotation (Placement(transformation(extent={{-72,32},{-52,
                52}}, rotation=0)));
    ...
    equation
      ...
      connect(PressDrop1.outlet, SinkP1.flange)
        annotation (Line(points={{54,30},{66,30}}, color={159,159,223}));
      connect(PressDrop2.outlet, Compressor11_1.inlet) annotation (Line(points=
              {{-52,42},{-50,42},{-50,30},{-48,30}}, color={159,159,223}));
      connect(SourceP1.flange, PressDrop2.inlet)
        annotation (Line(points={{-82,42},{-72,42}}, color={159,159,223}));
      connect(Compressor11_1.outlet, PressDrop3.inlet) annotation (Line(points=
    ...
    model Test_low1
      ...
      Modelica.Mechanics.Rotational.Components.Fixed Fixed1 annotation (
          Placement(transformation(extent={{-70,-14},{-50,6}}, rotation=0)));
      Modelica.Mechanics.Rotational.Components.Damper Damper1(d=9) annotation (
    ...
    equation
      ...
      connect(PressDrop1.outlet, SinkP1.flange)
        annotation (Line(points={{54,30},{66,30}}, color={159,159,223}));
      connect(PressDrop2.outlet, Compressor1.inlet) annotation (Line(points={{-52,
              42},{-50,42},{-50,30},{-48,30}}, color={159,159,223}));
      connect(SourceP1.flange, PressDrop2.inlet)
        annotation (Line(points={{-82,42},{-72,42}}, color={159,159,223}));
      connect(Compressor1.outlet, PressDrop3.inlet)
...
public */
      ...
      ThermoPower.Gas.Compressor Compressor(
        ...
        Table=ThermoPower.Choices.TurboMachinery.TableTypes.file,
        beta(start=5),
        fileName="D:/mati_nuovo/tabelle/COMPR.mat") annotation (Placement(
            transformation(extent={{-24,4},{-4,24}}, rotation=0)));
...
<p>Simulate for 2 seconds. The compressor slows down.
    ...
    equation
      ...
      connect(ConstantSpeed1.flange, Inertia1.flange_b)
        annotation (Line(points={{82,-16},{54,-16}}, color={0,0,0}));
      connect(SourceW1.flange, GTunit1.Fuel_in) annotation (Line(points={{-18,
              50},{-14,50},{-14,24.4},{-15,24.4}}, color={159,159,223}));
      connect(GTunit1.FlueGas_out, SinkP1.flange)
        annotation (Line(points={{5.7,16},{34,16}}, color={159,159,223}));
      connect(GTunit1.Air_in, SourceP1.flange)
        annotation (Line(points={{-35.7,16},{-80,16}}, color={159,159,223}));
    end GTprova_ISO11;
...
<p>Simulate for 2 seconds. The compressor slows down.
    ...
    protected
      parameter Modelica.SIunits.MassFraction Xturb[Medium.nX]={0.15581,0.01320,
          0.03822,0.05543,0.73734};
    ...
    public
      ThermoPower.Gas.SourcePressure
        ...
        p0=12.223e5,
        Xnom=Xturb,
        T=1267.33) annotation (Placement(transformation(extent={{-94,28},{-74,
                48}}, rotation=0)));
      Modelica.Mechanics.Rotational.Components.Inertia Inertia1(J=10000)
      ...
      Gas.SinkPressure
        ...
        Xnom=Xexhaust,
        p0=1.003e5,
        T=580) annotation (Placement(transformation(extent={{14,28},{34,48}},
              rotation=0)));
      Modelica.Mechanics.Rotational.Sources.ConstantSpeed ConstantSpeed1(
          w_fixed=Modelica.SIunits.Conversions.from_rpm(4923), useSupport=false)
    ...
    equation
      ...
      connect(PressDrop1.outlet, Turbine.inlet)
        annotation (Line(points={{-40,38},{-22.2,38}}, color={159,159,223}));
      annotation (
        extent=[-58, 20; -38, 40],
        Diagram(graphics),
        Documentation(info="<html>
