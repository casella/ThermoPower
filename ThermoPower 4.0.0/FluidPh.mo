...
<li><i>1 Oct 2003</i>
  ...
  connector FlangeA "A-type flange connector for p-h fluid flows"
    ...
    annotation(
      ...
      Diagram(graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillColor = {0, 0, 255}, fillPattern = FillPattern.Solid)}));
  end FlangeA;
  ...
  connector FlangeB "B-type flange connector for p-h fluid flows"
    ...
    annotation(
      ...
      Diagram(graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillColor = {0, 0, 255}, fillPattern = FillPattern.Solid), Ellipse(extent = {{-40, 40}, {40, -40}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid)}));
  end FlangeB;
  ...
  model SourcePressure "Pressure source for p-h fluid flows"
    ...
    parameter Boolean use_in_T = false "Use connector input for the temperature" annotation(
      Dialog(group = "External inputs", enable = not use_in_h),
      choices(checkBox = true));
    parameter Boolean use_in_h = false "Use connector input for the specific enthalpy" annotation(
      Dialog(group = "External inputs", enable = not use_in_T),
      choices(checkBox = true));
...
<li><i>1 Oct 2003</i> by <a href=\"mailto:francesco.casella@polimi.it\">Francesco Casella</a>:<br>First release.</li>
  ...
  model Flow1DFV "1-dimensional fluid flow model for p-h fluid (finite volumes)"
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFV2ph "1-dimensional fluid flow model for p-h fluid (finite volumes, 2-phase)"
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFEM "1-dimensional fluid flow model for p-h fluid (finite elements)"
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFEMnm "1-dimensional fluid flow model for p-h fluid (finite elements)"
    extends BaseClasses.Flow1DBase(Nw = N);
...
/* first equation
  ...
  */
    if regularizeBoundaryConditions then
      (rho[1]*(l/12)*(3 - 2*alpha_sgn[1]) + rho[2]*(l/12)*(1 - alpha_sgn[1]))*der(h[1]) + (rho[1]*(l/12)*(1 - alpha_sgn[1]) + rho[2]*(l/12)*(1 - 2*alpha_sgn[1]))*der(h[2]) + (((w[1]*(3*alpha_sgn[1] - 4) + w[2]*(3*alpha_sgn[1] - 2))/12)*h[1] + ((w[1]*(4 - 3*alpha_sgn[1]) + w[2]*(2 - 3*alpha_sgn[1]))/12)*h[2])/A + (Functions.stepReg(infl.m_flow - wnom*wnf_bc, (1 - alpha_sgn[1]/2)*w[1], 0, wnom*wnf_bc))*h[1]/A = (((4 - 3*alpha_sgn[1])*l/12)*phi[1] + ((2 - 3*alpha_sgn[1])*l/12)*phi[2])*(omega/A) + (Functions.stepReg(infl.m_flow - wnom*wnf_bc, (1 - alpha_sgn[1]/2)*inStream(infl.h_outflow), 0, wnom*wnf_bc))*w[1]/A + (if Medium.singleState then 0 else der(p)*(l/2*(1 - alpha_sgn[1])));
    else
      (rho[1]*(l/12)*(3 - 2*alpha_sgn[1]) + rho[2]*(l/12)*(1 - alpha_sgn[1]))*der(h[1]) + (rho[1]*(l/12)*(1 - alpha_sgn[1]) + rho[2]*(l/12)*(1 - 2*alpha_sgn[1]))*der(h[2]) + (((w[1]*(3*alpha_sgn[1] - 4) + w[2]*(3*alpha_sgn[1] - 2))/12)*h[1] + ((w[1]*(4 - 3*alpha_sgn[1]) + w[2]*(2 - 3*alpha_sgn[1]))/12)*h[2])/A + (noEvent(if infl.m_flow >= 0 then (1 - alpha_sgn[1]/2)*w[1] else 0))*h[1]/A = (((4 - 3*alpha_sgn[1])*l/12)*phi[1] + ((2 - 3*alpha_sgn[1])*l/12)*phi[2])*(omega/A) + (noEvent(if infl.m_flow >= 0 then (1 - alpha_sgn[1]/2)*inStream(infl.h_outflow) else 0))*w[1]/A + (if Medium.singleState then 0 else der(p)*(l/2*(1 - alpha_sgn[1])));
    end if;
/* Intermediate equations, if N>2
  ...
  */
    if N > 2 then
      for i in 2:(N - 1) loop
        (rho[i - 1]*(l/12)*(1 + 2*alpha_sgn[i]) + rho[i]*(l/12)*(1 + alpha_sgn[i]))*der(h[i - 1]) + (rho[i - 1]*(l/12)*(1 + alpha_sgn[i]) + rho[i]*(l/12)*6 + rho[i + 1]*(l/12)*(1 - alpha_sgn[i]))*der(h[i]) + (rho[i]*(l/12)*(1 - alpha_sgn[i]) + rho[i + 1]*(l/12)*(1 - 2*alpha_sgn[i]))*der(h[i + 1]) + (((-w[i - 1]*(2 + 3*alpha_sgn[i]) - w[i]*(4 + 3*alpha_sgn[i]))/12)*h[i - 1] + ((w[i - 1]*(2 + 3*alpha_sgn[i]) + w[i]*6*alpha_sgn[i] + w[i + 1]*(3*alpha_sgn[i] - 2))/12)*h[i] + ((w[i]*(4 - 3*alpha_sgn[i]) + w[i + 1]*(2 - 3*alpha_sgn[i]))/12)*h[i + 1])/A = (((2 + 3*alpha_sgn[i])*l/12)*phi[i - 1] + (8*l/12)*phi[i] + ((2 - 3*alpha_sgn[i])*l/12)*phi[i + 1])*(omega/A) + (if Medium.singleState then 0 else der(p)*l);
      end for;
    end if;
/* last equation
  ...
  */
    if regularizeBoundaryConditions then
      (rho[N - 1]*(l/12)*(1 + 2*alpha_sgn[N]) + rho[N]*(l/12)*(1 + alpha_sgn[N]))*der(h[N - 1]) + (rho[N - 1]*(l/12)*(1 + alpha_sgn[N]) + rho[N]*(l/12)*(3 + 2*alpha_sgn[N]))*der(h[N]) + (((-w[N - 1]*(2 + 3*alpha_sgn[N]) - w[N]*(4 + 3*alpha_sgn[N]))/12)*h[N - 1] + ((w[N - 1]*(2 + 3*alpha_sgn[N]) + w[N]*(4 + 3*alpha_sgn[N]))/12)*h[N])/A + (Functions.stepReg(outfl.m_flow - wnom*wnf_bc, -(1 + alpha_sgn[N]/2)*w[N], 0, wnom*wnf_bc))*h[N]/A = (((2 + 3*alpha_sgn[N])*l/12)*phi[N - 1] + ((4 + 3*alpha_sgn[N])*l/12)*phi[N])*(omega/A) + (Functions.stepReg(outfl.m_flow - wnom*wnf_bc, -(1 + alpha_sgn[N]/2)*inStream(outfl.h_outflow), 0, wnom*wnf_bc))*w[N]/A + (if Medium.singleState then 0 else der(p)*(l/2*(1 + alpha_sgn[1])));
    else
      (rho[N - 1]*(l/12)*(1 + 2*alpha_sgn[N]) + rho[N]*(l/12)*(1 + alpha_sgn[N]))*der(h[N - 1]) + (rho[N - 1]*(l/12)*(1 + alpha_sgn[N]) + rho[N]*(l/12)*(3 + 2*alpha_sgn[N]))*der(h[N]) + (((-w[N - 1]*(2 + 3*alpha_sgn[N]) - w[N]*(4 + 3*alpha_sgn[N]))/12)*h[N - 1] + ((w[N - 1]*(2 + 3*alpha_sgn[N]) + w[N]*(4 + 3*alpha_sgn[N]))/12)*h[N])/A + (noEvent(if outfl.m_flow >= 0 then -(1 + alpha_sgn[N]/2)*w[N] else 0))*h[N]/A = (((2 + 3*alpha_sgn[N])*l/12)*phi[N - 1] + ((4 + 3*alpha_sgn[N])*l/12)*phi[N])*(omega/A) + (noEvent(if outfl.m_flow >= 0 then -(1 + alpha_sgn[N]/2)*inStream(outfl.h_outflow) else 0))*w[N]/A + (if Medium.singleState then 0 else der(p)*(l/2*(1 + alpha_sgn[1])));
    end if;
...
The basic modelling assumptions are:
  ...
  model Flow1DFEM2ph "1-dimensional fluid flow model for p-h fluid (finite elements)"
    extends BaseClasses.Flow1DBase(Nw = N, redeclare replaceable package Medium = StandardWater "Medium model" constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium);
    replaceable model HeatTransfer = Thermal.HeatTransferFEM.IdealHeatTransfer constrainedby ThermoPower.Thermal.BaseClasses.DistributedHeatTransferFEM annotation(
       choicesAllMatching = true);
    HeatTransfer heatTransfer(redeclare package Medium = Medium, final Nf = N, final Nw = Nw, final Nt = Nt, final L = L, final A = A, final Dhyd = Dhyd, final omega = omega, final wnom = wnom/Nt, final w = w, final fluidState = fluidState) "Instantiated heat transfer model";
    ThermoPower.Thermal.DHTNodes wall(N = N) annotation(
...
<li><i>1 Oct 2003</i>
  ...
  model SensT1 "Temperature sensor for p-h fluid flows, single port"
    ...
    replaceable package Medium = ThermoPower.FluidPh.StandardWater constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    Modelica.Blocks.Interfaces.RealOutput T annotation(
      Placement(transformation(extent = {{60, 40}, {100, 80}}, rotation = 0)));
...
<li><i>26 Jul 2017</i> <a href=\"mailto:francesco.casella@polimi.it\">Francesco Casella</a>:<br>First release.</li>
  ...
  model CoolingTower "Cooling tower with variable speed fan"
    ...
    ThermoPower.FluidPh.FlangeA waterInlet annotation(
      Placement(transformation(extent = {{-10, 80}, {10, 100}}), iconTransformation(extent = {{-10, 80}, {10, 100}})));
    ThermoPower.FluidPh.FlangeB waterOutlet annotation(
...
<li><i>1 Oct 2003</i>
  ...
  function f_colebrook_2ph "Fanning friction factor for a two phase p-h fluid flow"
    extends Modelica.Icons.Function;
    input SI.MassFlowRate w;
    input Real D_A;
    input Real e;
...
<p>It is possible to use a generic initial equation instead of <code>p = pstart</code>, e.g., to set the initial charge of a refrigeration circuit, by setting <code>initEquationFromInput = true</code> and connecting to the input the output of a <a href=\"Modelica://Modelica.Blocks.Sources.RealExpression\">Modelica.Blocks.Sources.RealExpression</a> component, containing the residual of the initial equation. In this case, if <code>useHomotopy = true</code>, the simplified initial equation is <code>(p - pstart)/pstart = 0</code>. To ensure a smooth homotopy transformation, it is recommended to write the residual in normalized form, i.e., with values have the order of magnitude of one, as for the simplified equation.</p>
  ...
  package BaseClasses "Contains partial models"
    ...
    partial model Flow1DBase "Basic interface for 1-dimensional p-h fluid fluid flow models"
      import ThermoPower.Choices.Flow1D.FFtypes;
      replaceable package Medium = StandardWater constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
         choicesAllMatching = true);
...
<li><i>5 Oct 2011</i>
    ...
    partial model EvaporatorBase "Basic interface for 1-dimensional p-h fluid fluid flow models"
